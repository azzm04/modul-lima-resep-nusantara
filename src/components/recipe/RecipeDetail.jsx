...
// --- Tambahkan logika share ---
const handleShare = (recipe) => {
  const shareData = {
    title: recipe.name,
    text: `Cek resep "${recipe.name}" di Resep Nusantara!`,
    // Gunakan URL saat ini sebagai fallback, ganti dengan URL publik PWA jika sudah deploy
  ...
  if (navigator.share) {
    ...
    navigator.share(shareData)
      .then(() => console.log('Resep berhasil dibagikan'))
      .catch((error) => console.error('Error saat berbagi', error));
  } else {
    // Fallback: Copy to clipboard
    navigator.clipboard.writeText(`${shareData.text} ${shareData.url}`)
      .then(() => alert('Link resep berhasil disalin ke clipboard!'))
...
export default function RecipeDetail({ recipeId, onBack, onEdit, category = 'makanan' }) {
  ...
  // favorite state handled by `FavoriteButton` component; remove unused hook
  ...
  const handleSubmitReview = async (e) => {
    ...
    const reviewData = {
      user_identifier: userProfile.username || userId,
      rating,
    ...
    if (success) {
      // Save review to localStorage for profile page
      try {
        const storageKey = `reviews_${userId}`;
        const reviews = JSON.parse(localStorage.getItem(storageKey) || '[]');
        const newReview = {
          id: Date.now().toString(), // Unique ID untuk keperluan edit/hapus
          recipeId,
        ...
        // Trigger event untuk memperbarui ProfilePage
        window.dispatchEvent(new CustomEvent('reviewsChanged'));
      } catch (error) {
        console.error('Error menyimpan ulasan ke localStorage:', error);
      }
      ...
      <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => handleShare(recipe)} // <-- Tombol Share di sini
              className="flex items-center gap-2 px-4 py-2 bg-gray-200 text-slate-700 rounded-lg hover:bg-gray-300 transition-colors"
              title="Bagikan Resep"
            >
              <Share2 className="w-4 h-4" />
              <span className="hidden md:inline">Bagikan</span>
            </button>
            {onEdit && ( // Tombol Edit & Hapus (Opsional, hanya jika prop onEdit ada)
              <>
                <button
                  onClick={() => {
                      ...
                      <div className="text-right">
                        ...
                        {review.edited_at && (
                          <p className="text-xs text-slate-400 mt-1">
                            (Diedit: {formatDate(review.edited_at)})
                          </p>
                        )}
                      </div>
                    </div>
