    ...
    const loadRecipe = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          // Set form data dengan semua field
          setFormData({
            name: recipe.name || '',
          ...
          // Set ingredients - Handle berbagai format
          ...
          // Set steps - Handle berbagai format (array of objects atau array of strings)
          if (recipe.steps && Array.isArray(recipe.steps) && recipe.steps.length > 0) {
            const stepsArray = recipe.steps.map(step => {
              // Jika step adalah object dengan property 'instruction' atau 'step'
              if (typeof step === 'object') {
                return step.instruction || step.step || '';
              }
              // Jika step sudah string
              return typeof step === 'string' ? step : '';
            }).filter(step => step.trim() !== ''); // Filter empty steps
        ...
        alert('Gagal memuat data resep: ' + err.message);
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (uploadResult.success) {
          ...
          console.log('✅ Image uploaded:', uploadResult.data.url);
        } else {
        ...
        console.log('📷 Keeping existing image:', currentImageUrl);
      }

      // Step 2: Add ingredients and steps
      const validIngredients = ingredients.filter(ing => ing.name.trim() && ing.quantity.trim());
      const validSteps = steps.filter(step => step.trim());
